...
if ( isset( $_POST['cancel'] ) ) {
      header("Location: ".$hostpath."/chargetypeList.php?res=01&msg='New Entry'&id=''&mod=5");
}
else
{
    if ( isset( $_POST['add'] ) ) {
...
*/
        $qry="insert into chargetype( `title`,`description`, `makedt`, `makeby`) 
    ...
    if ( isset( $_POST['update'] ) ) {
      ...
      $description= $_REQUEST['description'];
        ...
        $qry="update chargetype set `title`='".$title."',`description`='".$description."'  where `id`=".$aid."";
    ...
    if ($conn->query($qry) == TRUE) {
                header("Location: ".$hostpath."/chargetypeList.php?pg=1&mod=5");
    } else {
