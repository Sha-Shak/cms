...
if ( isset( $_POST['cancel'] ) ) {
      header("Location: ".$hostpath."/paymodeList.php?res=01&msg='New Entry'&id=''&mod=5");
}
else
{
    if ( isset( $_POST['add'] ) ) {
...
*/
        $qry="insert into paytype( `title`, `descr`, `makeby`, `makedt`) 
    ...
    if ( isset( $_POST['update'] ) ) {
      ...
      $description= $_REQUEST['description'];
        ...
        $qry="update paytype set `title`='".$title."',`descr`='".$description."' where `id`=".$aid."";
    }
    ...
    if ($conn->query($qry) == TRUE) {
                header("Location: ".$hostpath."/paymodeList.php?pg=1&mod=5");
    } else {
