...
<!-- End Grid Status Menu -->
                ...
                <div >
                    ...
                    <table id='listTable' class='display dataTable' width="100%">
                        <thead>
                        <tr>
                            <th data-class-name="priority">Sl</th>
                            <th>Icon</th>
     ...
     <!-- Datatable JS -->       
        ...
        $(document).ready(function(){
            $('#listTable').DataTable({
				...
				scroller: true,
                'ajax': {
                    'url':'phpajax/data_all_list.php?action=boosttpst'
                },
                'columns': [
                    { data: 'id' },
                    { data: 'photo' },
...
function update_grid_status_menu(thisvalue,id, status_id){
	var dealdata = { dataid:id,statusid: status_id, modulename : 'boosttype', colname : 'status', selectedvalue : thisvalue}
	var saveData = $.ajax({
		  type: 'POST',
		  url: "phpajax/update_obj_status.php?action=changeboosttpstatus",
		  data: dealdata,
		  dataType: "text",
